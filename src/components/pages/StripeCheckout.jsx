...
const CheckoutForm = ({ amount, items, address, onSuccess }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const { error, paymentIntent } = await stripe.confirmPayment({
      elements,
      confirmParams: {
        return_url: "https://quick-cart01-eight.vercel.app/confirm-order",
      },
    ...
    if (error) {
      ...
      try {
        const response = await fetch(
          "https://quickcart-02mk.onrender.com/api/payment/confirm-order",
          {
            method: "POST",
            headers: {
...
const StripeWrapper = ({ amount, items, address, onSuccess }) => {
  ...
  React.useEffect(() => {
    ...
    fetch("https://quickcart-02mk.onrender.com/api/payment/create-payment-intent", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("token")}`,
